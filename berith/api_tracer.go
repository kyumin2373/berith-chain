...
func (api *PrivateDebugAPI) TraceChain(ctx context.Context, start, end rpc.BlockNumber, config *TraceConfig) (*rpc.Subscription, error) {
	...
	case rpc.PendingBlockNumber:
		from = api.e.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		from = api.e.blockchain.CurrentBlock()
	default:
		from = api.e.blockchain.GetBlockByNumber(uint64(start))
	}
	switch end {
	case rpc.PendingBlockNumber:
		to = api.e.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		to = api.e.blockchain.CurrentBlock()
	default:
		to = api.e.blockchain.GetBlockByNumber(uint64(end))
	}
...
func (api *PrivateDebugAPI) traceChain(ctx context.Context, start, end *types.Block, config *TraceConfig) (*rpc.Subscription, error) {
	...
	database := state.NewDatabaseWithCache(api.e.ChainDb(), 16) // Chain tracing will probably start at genesis

	if number := start.NumberU64(); number > 0 {
		start = api.e.blockchain.GetBlock(start.ParentHash(), start.NumberU64()-1)
		if start == nil {
		...
		go func() {
			...
			for task := range tasks {
				...
				for i, tx := range task.block.Transactions() {
					...
					isBIP4 := api.config.IsBIP4(end.Header().Number)
...
func (api *PrivateDebugAPI) TraceBlockByNumber(ctx context.Context, number rpc.BlockNumber, config *TraceConfig) ([]*txTraceResult, error) {
	...
	switch number {
	case rpc.PendingBlockNumber:
		block = api.e.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		block = api.e.blockchain.CurrentBlock()
	default:
		block = api.e.blockchain.GetBlockByNumber(uint64(number))
	}
...
func (api *PrivateDebugAPI) traceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*txTraceResult, error) {
	...
	if err := api.e.engine.VerifyHeader(api.e.blockchain, block.Header(), true); err != nil {
		return nil, err
	}
	parent := api.e.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
...
func (api *PrivateDebugAPI) computeStateDB(block *types.Block, reexec uint64) (*state.StateDB, error) {
	...
	database := state.NewDatabaseWithCache(api.e.ChainDb(), 16)

	for i := uint64(0); i < reexec; i++ {
		block = api.e.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
		if block == nil {
			break
		}
	...
	for block.NumberU64() < origin {
		...
		if block = api.e.blockchain.GetBlockByNumber(block.NumberU64() + 1); block == nil {
...
func (api *PrivateDebugAPI) TraceTransaction(ctx context.Context, hash common.Hash, config *TraceConfig) (interface{}, error) {
	...
	tx, blockHash, _, index, _, _ := rawdb.ReadTransaction(api.e.ChainDb(), hash)
	if tx == nil {
