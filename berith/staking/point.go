...
*/
func CalcAdvForExceededPoint(nowBlockNumber, stakeBlockNumber *big.Int, period uint64, stakeMaximumInBer *big.Float) *big.Int {
	d := float64(period) / 10 //공식이 10초 단위 이기때문에 맞추기 위함 (perioid 를 제네시스로 변경하면 자동으로 변경되기 위함)
	...
	*/
	...
	temp1 := new(big.Float).Quo(stakeMaximumInBer, new(big.Float).Add(stakeMaximumInBer, big.NewFloat(0)))
	temp2 := new(big.Float).Mul(stakeMaximumInBer, temp1)
	temp3 := new(big.Float).Mul(temp2, ratio)
	adv := new(big.Int)
	new(big.Float).Quo(temp3, big.NewFloat(100)).Int(adv)
