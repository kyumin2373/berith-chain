...
func CalcPointBigint(pStake, addStake, now_block, stake_block *big.Int, period uint64, isBIP4 bool) *big.Int {
	p := pStake //이전스테이킹
	n := addStake //추가스테이킹
...
*/
func CalcAdvForExceededPoint(nowBlockNumber, stakeBlockNumber *big.Int, period uint64) *big.Int {
	stakeMaximum, _ := new(big.Float).SetString(params.StakeMaximum)
	stakeMaximum = new(big.Float).Quo(stakeMaximum, big.NewFloat(1e+18))
	...
	*/
	...
	temp1 := new(big.Float).Quo(stakeMaximum, new(big.Float).Add(stakeMaximum, big.NewFloat(0)))
	temp2 := new(big.Float).Mul(stakeMaximum, temp1)
	temp3 := new(big.Float).Mul(temp2, ratio)
	adv := new(big.Int)
	new(big.Float).Quo(temp3, big.NewFloat(100)).Int(adv)
...
*/
func checkMaxStakeBalance(stake *big.Int) *big.Int {
