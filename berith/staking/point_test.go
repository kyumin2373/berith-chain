...
func TestCalcPointBigint(t *testing.T) {
	...
	for _, test := range tests {
		result := CalcPointBigint(test.prev_stake, test.add_stake, test.new_block, test.stake_block, stakeMaximum, test.perioid, test.isBIP4)
...
func TestCheckMaxStakeBalance(t *testing.T) {
	...
	for _, testData := range testDatas {
		result := checkMaxStakeBalance(testData.point, stakeMaximum)
