...
func SelectBlockCreator(config *params.ChainConfig, number uint64, hash common.Hash, stks staking.Stakers, state *state.StateDB) VoteResults {
	...
	for _, stk := range list {
		...
		*/
		var point uint64
		if config.IsBIP4(blockNumber) && stakeBalance.Cmp(config.Bsrr.StakeMaximum) == 1  {
			stakeMaximumInBer := new(big.Int).Div(config.Bsrr.StakeMaximum, big.NewInt(1e+18))
			lastStkBlock := new(big.Int).Set(state.GetStakeUpdated(stk))
			adv := staking.CalcAdvForExceededPoint(blockNumber, lastStkBlock, config.Bsrr.Period, common.BigIntToBigFloat(stakeMaximumInBer))

			point = new(big.Int).Add(stakeMaximumInBer, adv).Uint64()
		} else {
			point = state.GetPoint(stk).Uint64()
		}
